...
def main(settings, problems_geojson):
    ...
    if len(admin_files) < 2:
        ...
        for layer, feature in lyr_read.readData():
            osm_id = feature.GetField('GID_1')
            name = feature.GetField('name_1')
            name_en = feature.GetField('name_1')
            geom_raw = ogr.ForceToMultiPolygon(feature.GetGeometryRef())
    ...
    if len(admin_files) < 3:
        ...
        for layer, feature in lyr_read.readData():
            ...
            osm_id = feature.GetField('GID_2')
            name = feature.GetField('name_2')
            name_en = feature.GetField('name_2')
            geom_raw = ogr.ForceToMultiPolygon(feature.GetGeometryRef())
